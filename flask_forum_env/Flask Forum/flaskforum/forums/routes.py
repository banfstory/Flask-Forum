...
from flaskforum.models import Post, Forum, Follow_Forum, Comment, Reply
...
def forum(name):
    ...
    follow_forum = Follow_Forum.query.filter(Follow_Forum.forum_id==forum[0].id).filter(Follow_Forum.user_id==current_user.id).first() if current_user.is_authenticated else None
    ...
    return render_template('forum.html', follow=follow, posts=posts, forum=forum, searchForm=searchForm, follow_forum=follow_forum, title=forum[0].name)
...
def comments(id):
    ...
    follow = Follow_Forum.query.filter_by(user_id=current_user.id) if current_user.is_authenticated else None
    return render_template('comments.html', follow_forum=follow_forum, follow=follow, comment=comment, reply=reply, post=post, post_comments=post_comments, searchForm=searchForm, id=id, replyForm=replyForm, title= post[0].title + " - Post", forum_date=forum_date, res_reply=res_reply)
...
def post_comment(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
    return redirect(url_for('forums.comments', id=id, searchForm=searchForm))
...
def reply_comment(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
    return redirect(url_for('forums.comments', id=comment.comment_post.id, searchForm=searchForm))
...
def update_comment(id): 
    ...
    if form.validate_on_submit(): 
        ...
        db.session.commit()
    return redirect(url_for('forums.comments', id=comment.comment_post.id))
...
def update_reply(id): 
    ...
    if form.validate_on_submit(): 
        ...
        db.session.commit()
    return redirect(url_for('forums.comments', id=reply.reply.comment_post.id))