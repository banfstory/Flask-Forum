...
from flaskforum.models import User, Post, Forum, Follow_Forum, Comment
...
def follow(id):
    ...
    follow_forum = Follow_Forum.query.filter(Follow_Forum.forum_id==id).filter(Follow_Forum.user_id==current_user.id).first() if current_user.is_authenticated else None
    if not follow_forum:
        current_user.forums_followed+=1
        forum.followers+=1
        follow = Follow_Forum(user_id=current_user.id, forum_id=id)
...
def unfollow(id):
     ...
     if follow_forum:
        current_user.forums_followed-=1
        forum.followers-=1
        db.session.delete(follow_forum)
        db.session.commit()
     return redirect(url_for('forums.forum', name=forum.name))
