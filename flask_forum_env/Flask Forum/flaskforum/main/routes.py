...
from flaskforum.models import Post, Follow_Forum, Comment
...
def home():
    ...
    posts = db.session.query(Post,(db.func.julianday('now')-db.func.julianday(Post.date_posted))*24,db.func.count(Comment.post_id)).order_by(Post.date_posted.desc()).outerjoin(Comment, Post.id==Comment.post_id).group_by(Post.id).group_by(Post.id).paginate(page=page, per_page=10)
