...
def api_create_post(c_user):
    ...
    post = Post(title=title, content=content,
                forum_id=forum_id, user_id=c_user.id)
    db.session.add(post)
    forum.num_of_post += 1
    db.session.commit()
...
def api_get_posts():
    ...
    try:
        posts = Post.query.filter_by(
            **param).order_by(Post.date_posted.desc()).paginate(page=page, per_page=per_page).items
    except:
        return jsonify({'message': 'Invalid request'})
...
def api_get_comments():
    ...
    try:
        comments = Comment.query.filter_by(
            **param).order_by(Comment.date_commented.desc()).paginate(page=page, per_page=per_page).items
    except:
        return jsonify({'message': 'Invalid request'})
...
def api_get_replys():
    ...
    try:
        replys = Reply.query.filter_by(
            **param).order_by(Reply.date_reply.desc()).paginate(page=page, per_page=per_page).items
    except:
        return jsonify({'message': 'Invalid request'})
...
def api_get_forums():
    ...
    try:
        forums = Forum.query.filter_by(
            **param).order_by(Forum.date_created.desc()).paginate(page=page, per_page=per_page).items
    except:
        return jsonify({'message': 'Invalid request'})
